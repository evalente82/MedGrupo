    ...
    public class ValidaContato(Contato obj)
    {
        public static ErroViewModel ValidarDados(ContatoViewModel obj)
        {
            ...
            if (obj.DtNascimento > DateTime.Now)
                return new ErroViewModel() { Valido = false, Erro = "Data de nascimento é maior que a data atual" };
            if (obj.Idade < 18)                
                return new ErroViewModel() { Valido = false, Erro = "O contato deve ser maior de idade" };
